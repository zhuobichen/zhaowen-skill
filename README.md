...
| Skill | 目录 | 用途 |
|-------|------|------|
| docx 转公众号（clz） | [`clz_docx_to_mp/`](clz_docx_to_mp/) | docx → 公众号图文三步流水线：解析、藏青蓝渲染、批量推草稿箱（43 图实测通过） |
...
| 公众号后台操作（clz） | [`clz_wechat_mp_ops/`](clz_wechat_mp_ops/) | 微信公众号后台：草稿读写、正文 dispatch 改字、批量替换配图、四层发布前校验 |
