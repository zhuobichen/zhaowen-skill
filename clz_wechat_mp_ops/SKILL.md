...
- **主席/召集人/主持人（特殊）**：**先全部中文（3行）再全部英文（3行）**，中间留小分隔（不用逐行对应）
- **中英来源（以数据表为主）**：优先用 xlsx 本身的中英（v13+ 数据表已含完整中英，**无需翻译表**）；仅**主席英文用第四轮映射兜底**（xlsx 主席英文可能缺 Conveners/Moderators 标签，映射有完整标签）；数据源都没有的 → 不补（留中文）
- **6 列错位表格**（如分会场12）：时间/题目/题目重复/报告人+英文/重复/单位+英文 → 用 列0/1/3/5 提取
- **信息行**：每张图含 标题栏（中英主题）+ 时间/地点（中英）+ 主席/召集人/主持人（先中后英）+ 完整日程（题目/报告人/单位中英）

### 中英解析要点（踩坑，务必遵守）
- **正则字符类用 `[一-鿿]`**：用 `[一-鿿]` 在某些环境**匹配失败**（re.match 返回 None 导致中文拆不出）
- **`\r\n` 归一化**：解析前 `s.replace("\r\n","\n").replace("\r","\n")`，否则 `(.*)$` 因 `\r\n` 跨行失败
- **中英混行分离**（同一单元格中英在同一行，非换行）：依次尝试
  - 换行/`/`/分号 `;`/全角 `；` 分隔
  - `中文 + 空格 + 英文`（正则 `^(.*[一-鿿])\s+([A-Za-z].*)$`）
  - 中文/数字后空格 + 字母或数字开头英文（如 `3D Detection`、`2025-2030 California`）
  - 中文后全角括号/空格 + 英文
- 中英可能粘连无空格（如 `潘小乐Xiaole Pan`、`佟胜睿Shengrui Tong`）→ 报告人先归一化空格再拆

### 从会议议程 docx 生成（跳过主会场）
`gen_agenda_v02_docx.py <会议议程docx>`：解析 docx 表格2~13（**表格1=主会场/9月14日，跳过**），生成分会场1~12 中英双语日程图（复用 gen_session_agenda_bi 渲染）。docx 表格每格中英分行，字段提取同 xlsx。
