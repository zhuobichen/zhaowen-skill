...
### ⚠️⚠️ 稿子级的改动必须落盘成 sidecar，否则两个入口会不一致

**`push_to_mp.py` 不读 `article.html`**——它自己从 `content.json` 重新 `render_blocks` 一遍。
所以任何"只改渲染脚本"的改动，**推送时都不生效**。

> 已踩：给 `render_wechat.py` 加了 `--drop-image` 删掉一张重复照片，
> 本地预览确实删了，**推上去那张还在**。
> 露馅的方式很隐蔽——两版推完的**分批字节数一模一样**（479/852/935…），
> 正常删掉一张图不可能字节数全等。而且脚本仍然报「✅ 校验通过」，
> **因为它校验的是自己发过去的东西，没有跟你的改动对过账。**

现在统一走 **build 目录下的 sidecar 文件**，两个脚本都读同一份：

| 文件 | 内容 | 谁写 | 谁读 |
|---|---|---|---|
| `<build>/drop_images.txt` | 本稿不用的图（一行一个，**不含扩展名**） | `render_wechat.py --drop-image` | 两个脚本都读 |
| `<build>/title.txt` | 标题覆盖 | `push_to_mp.py --title` | `push_to_mp.py` |

```bash
# 删掉一张重复照片（两个脚本都会照做）
python $SKILL/render_wechat.py <build_dir> --drop-image 027_image27

# 改标题（会落盘，之后重推自动沿用，不会弹回 docx 原标题）
python $SKILL/push_to_mp.py <build_dir> --token T --session S --title "……在广州成功召开"
```

> **通则**：凡是「这一篇稿子的决定」（删哪张图、标题写什么），
> 都要落盘成 build 目录里的文件，不能只存在于某次命令行参数里——
> 否则下次重推就会静默恢复原样。
...
正文里插了 GIF 装饰件（小鸟排在开篇卡片之前）后，**封面选择器会把这些 GIF 也列进去**，
而且它们排在前面。**微信的裁剪工具处理 GIF 会产出纯黑图**——封面设了、URL 也有，
但内容是黑的（实测 466×198 平均 RGB `[0,0,0]`）。

> 判断「封面是黑的」的办法：把封面 URL 拉下来量平均像素。别只看「有没有 background-image」——
> 我最初的检查就只判断属性存在，结果是假阳性，白白让用户发现。

**坑 2：选了封面后要等裁剪预览把图渲染出来再点「确认」。**

选图 → 「下一步」→ 等 **10-15 秒**（大图渲染慢）→ 才点「确认」。
抢时间点确认，可能在空 canvas 上裁出黑图。

**正确做法**：从正文选封面时，**按显示宽度挑一张 JPG**（我们的装饰 GIF 是 46px、正文图 578px），
不要盲选索引 0。脚本里可按「宽 > 100 的第一张」定位。

> 另注：**服务号的封面比例与订阅号不同**——服务号裁剪框是 `1:1（服务号列表）` + `2.35:1（服务号会话）`，
> 订阅号是 `2.35:1（消息列表）` + `1:1（转发卡片和公众号主页）`。
...
### ⚠️ 滚动条「加粗」和「常驻」都做不到（三条路全部实测堵死）

结论：**正文里的滚动条只能维持现状**。别再试了，以下都是本机实测结果。

**1. `scrollbar-width` 只有三个关键字，没有数值**

想「加粗 1.5 倍」是不可能的——它只接受 `thin` / `auto` / `none`。而且实测反而是
**`thin` 最粗**：
...
| `::-webkit-scrollbar{height:14px}` | 14px ← 唯一能更粗的路 |

**2. `::-webkit-scrollbar` 需要 `<style>`，而微信无差别剥除**

实测三次，全部失败：

| 尝试 | 结果 |
|---|---|
| 正文里放 `<style>` | ❌ 被剥（写的 `color:rgb(255,0,0)` 完全没生效） |
| `<svg>` 内部放 `<style>`（想借 SVG 的自带样式能力绕开） | ❌ **同样被剥**（`foreignObject` 留下来了，但 `<style>` 没了） |
| 内联 `style` 属性 | ❌ 定义不了伪元素 |

**3. 「浮层滚动条」是操作系统行为，CSS 无权覆盖**

Windows 的「自动隐藏滚动条」、macOS 默认、以及所有移动端，都会用浮层滚动条
（平时藏、滑动时短暂浮现）。**唯一能强制切回经典常驻型的手段就是 `::-webkit-scrollbar`**，
而它被第 2 条堵死。所以常驻也做不到——任何公众号都一样。

> 附带发现：**`id` 属性不会被剥**（`getElementById` 能取到），
> 但用它做的锚点跳转在编辑器里**点击无效**（实测 `scrollLeft` 不变），发表后是否有效未验证。
...
### ⚠️⚠️ 裁统一比例时**不能居中裁**，会切掉人物头部

为了让相册等高要裁竖图（见上）。**裁剪窗口的垂直位置不能取 0.5（居中）**——
演讲类照片人脸多在画面**偏上**，居中裁正好切头。

> 实测：李向东那张竖图 866×1254，人脸在 y=334-449；
> 居中窗口从 `(1254-577)/2 = 338` 开始——**差 4 像素就切了头**。
> 同一批里刘炳江那张居中裁却是好的（他的人脸在画面中下部 510-650，落在窗口内）。
> **所以「同一套参数对某些图有效」是陷阱：必须逐张看，不能只看一张就认为规则没问题。**

默认 `CROP_BIAS = 0.35`（偏上，优先保头）：

```python
top = int((h - nh) * bias)   # 0=贴顶，0.5=居中，1=贴底
```

**个别图构图特殊，用 `CROP_BIAS_BY_FILE` 逐张覆盖**（键是**不含扩展名**的文件名）：

```python
CROP_BIAS_BY_FILE = {
    # 屏幕上的英文占了大半幅，按默认裁完人只剩个头。
    # 窗口下移切掉英文、多露讲台。0.65 时头顶留 41px；0.70 只剩 5px，贴边太险。
    '015_image15': 0.65,
}
```

> ⚠️⚠️ **键必须用 stem，不能用带扩展名的全名。**
> `compress_images` 会把 `.jpeg` 压成 `.jpg`，而 `gallery_variant` 拿到的是压缩后的路径——
> 用 `'015_image15.jpeg'` 做键**永远匹配不上**，会静默退回默认值。
> 踩过：改完参数重渲，看图还是老样子，查了半天才发现键没生效。
> **参数改完必须看渲染产物，不能只看脚本"成功"了。**

实测余量（**别从缩放图估人脸位置，会偏 20px**）：
李向东那张在 `0.45` 时窗口 305-882，头顶实际在 y≈313——**只剩 8px，已接近极限**，`0.48` 就切头；
`0.35` 时留 76px，稳妥。刘炳江在 `0.65` 时留 41px。
**同一常量对两张图的安全边界差这么多——改这个值时必须逐张看图，不能只看一张就认为规则没问题。**
**换了别的体裁（风景/合影/文字截图）要重新定这个值，并逐张复核。**

> 更稳的做法（未采用，因为会改变版式）：**竖图不裁，直接拿出相册单独平铺**——
> 零内容损失，代价是相册被拆成两段。
...
> ⚠️ **校验脚本报「通过」不等于改动生效。**
> 它校验的是**脚本实际发过去的内容**；如果你改的是本地文件/参数而脚本没读到，
> 它会照样报通过。已踩两次：
> ① `--drop-image` 只加在渲染侧，推送侧重新渲染了一遍，照片没删掉；
> ② 逐图裁剪的键用了带扩展名的全名，匹配不上，静默退回默认值。
> **判断改动有没有生效，要看产物本身**——分批字节数变没变、把图下载下来看构图。
...
12. **外链图片（`<img src="外部地址">`）**：微信**会**自己抓取并转存到自家 CDN，
   但**绕不过 1080 上限**，而且抓不到的图会**静默阻断保存**。
   ⚠️ 最坑的是**保存前在编辑器里看，大图显示的是原始尺寸**，会让人误判成"外链能保住原图"。
   **验证尺寸必须走完「保存 → 重新打开 → 把图从 CDN 下载下来量」三步。**
   详见 `references/platform-limits.md` 第七节
13. **`--drop-image` / `--title` 改完必须重推**：这两个是**稿子级的决定**，落盘在 build 目录，
   但**改完不重推，草稿箱里还是旧的**。改完要重跑 `push_to_mp.py`（带 `--appmsgid` 复用同一篇）
14. **草稿 ID 会在保存后变化**：草稿箱里的 `appmsgid` 不是恒定的，我们经历过
   `100002724` → `100003340` 的变化，旧链接打开会提示「此草稿已被删除」。
   **不要长期缓存草稿链接**——每次都从草稿箱列表重新取 `data-appid`
